...
class DriverProxy(object):
    ...
    def __init__(self, engine, driverName, debug):
        ...
        print(f"### DEBUG : driver name {name} module {self._module} driver {self._driver}")
        # initialize refs
        self._engine = engine
        self._queue = []
        self._busy = True
    ...
    def _push(self, mtd, args, name=None):
        ...
        @type name: str
        '''
        print(f"\nDEBUG : driver _push mtd {mtd} args {args} name {name}")
        self._queue.append((mtd, args, name))
        print(f"DEBUG : driver queued {self._queue}")
        self._pump()
        # print(f"DEBUG : sleeping....")
        # sleep(10)
    ...
    def _pump(self):
        ...
        '''
        print(f"DEBUG : inside driver _pump method\n")
        # sleep(10)
        while (not self._busy) and len(self._queue):
            cmd = self._queue.pop(0)
            self._name = cmd[2]
            try:
                print(f"*********** DEBUG : driver _pump cmd {cmd} name {self._name} cmd[0] {cmd[0]} cmd[1] {cmd[1]}")
                cmd[0](*cmd[1])
                ...
                if self._debug:
                    traceback.print_exc()
        print(f"*********** DEBUG : driver after executing cmd")
    ...
    def setProperty(self, name, value):
        ...
        @type value: object
        '''
        print(f"DEBUG : driver setProperty name {name} value {value}")
