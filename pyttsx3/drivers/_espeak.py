...
def cfunc(name, dll, result, *args):
    ...
    print(f"^^^^^^^^^^^^^^^^^^^^^^^^^^ DEBUG : 11 result {result} \n atypes {atypes} \nname {name} \ndll {dll} \naflags {aflags}\n")
    return CFUNCTYPE(result, *atypes)((name, dll), tuple(aflags))

dll = cdll.LoadLibrary('libespeak.so.1')
# dll = cdll.LoadLibrary('libespeak.so.1.1.48')
...
print(f"DEBUG : 33 cListVoices {cListVoices}")
...
SetVoiceByName = cfunc('espeak_SetVoiceByName', dll, c_int,
                       ('name', c_char_p, 1))
print(f"&&&&&&&&&&&&&&&&&&& Till here is cleared DEBUG : 4 _espeak SetVoiceByName {SetVoiceByName}")
...
print(f"DEBUG : _espeak SetVoiceByProperties {SetVoiceByProperties}")
...
print(f"DEBUG : _espeak GetCurrentVoice {GetCurrentVoice}")
